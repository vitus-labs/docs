...
### Engine-Agnostic Result Type

`css()`'s return type is `CSSEngineResult` — an empty interface exposed by `@vitus-labs/core` that connectors augment via TypeScript module declaration merging. Each connector declares its concrete result shape so consumers' `` css`...` `` calls type-check against whichever engine they `init()`'d with, without `core` depending on any engine package. See [Core › CSSEngineResult](/docs/core#csseengineresult-augmentable-type) for details.
...
For React Native, use `connector-native` which parses CSS template literals into style objects instead of generating CSS class names. See the [dedicated React Native connector page](/docs/connectors/native) for details.
