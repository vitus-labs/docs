...
**Properties:** `fontSize`, `gap`, `rowGap`, `columnGap`, `gridAutoRows`, `gridAutoColumns`

> `lineHeight` and `letterSpacing` are `simple` properties (no conversion). Numbers pass through unchanged so `lineHeight: 1.5` becomes `line-height: 1.5` (a multiplier, not `1.5rem`).
