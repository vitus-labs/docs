...
**Object values** — null/undefined breakpoints carry forward the most recently seen value (same propagation rule as arrays):
...
// { fontSize: { xs: undefined, sm: 14, md: 14, lg: 18 } }
//                                     ↑ md inherits from sm
```

`xs` stays `undefined` because there is no prior breakpoint to inherit from.

### transformTheme()

Pivots from property-centric to breakpoint-centric. Pure pivot — every breakpoint is emitted, with each property keyed under its breakpoint:
...
Deduplication of consecutive identical breakpoints happens in a separate `optimizeTheme()` step — `makeItResponsive` chains both (`transformTheme` → `optimizeTheme`) so consumers get the optimized output by default. Call `transformTheme` directly only if you need the raw pivot.
