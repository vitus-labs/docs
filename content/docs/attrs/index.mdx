...
Both are bundled into a single `useRef({ $attrsRef, ref })` object that's forwarded to the inner render component, so all ref sources point to the same DOM node without any `useImperativeHandle` indirection.
