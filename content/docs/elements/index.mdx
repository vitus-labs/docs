...
### IteratorProps (generic Props\<T\>)

The Iterator's public type is a generic `Props<T>` that discriminates on the element type of `data`:

```tsx
type Props<T = unknown> = unknown extends T
  ? LooseProps                       // no narrowing → today's loose backwards-compatible shape
  : T extends string | number
    ? SimpleProps<T>                 // primitive items → valueName REQUIRED
    : T extends ObjectValue
      ? ObjectProps<T>               // object items → valueName FORBIDDEN, itemKey is keyof T | (item) => …
      : ChildrenProps                // no data, just children
```

`T` is inferred at the JSX call site from the `data` array, so consumers usually don't write the generic explicitly:

```tsx
<List data={['a', 'b']} component={Tag} valueName="label" />          // T inferred as string
<List data={[{ id: 1, name: 'Alice' }]} component={Row} itemKey="id" />  // T inferred as object, itemKey gets keyof T
```

The legacy `LooseProps` shape (the union of everything) is the fallback when `T` can't be narrowed — existing call sites continue to type-check without changes.
