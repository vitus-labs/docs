...
Reads `offsetWidth`/`offsetHeight` (depending on `direction`) on each slot and sets the smaller one to `Math.max(beforeSize, afterSize)` via direct style manipulation. The initial measurement runs in `useLayoutEffect` when `equalBeforeAfter`, `beforeContent`, `afterContent`, or `direction` change. After that, a `ResizeObserver` re-balances the slots whenever the element resizes — so the values stay correct without forcing a layout flush on every re-render. Falls back to the layout-effect-only path when `ResizeObserver` is unavailable.
