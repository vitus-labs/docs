...
> **Component swap resets attrs / priorityAttrs / compose / filterAttrs.**
> When the new `component` differs from the current one, these chain values
> are dropped — they were tailored to the previous component's prop shape and
> would silently leak invalid props onto the new component. Themes,
> dimensions, statics, and pseudo configuration are preserved (they describe
> the styling system, not the component's prop shape). Same-component swaps
> are a no-op for attrs. Re-chain `.attrs()`, `.compose()`, etc. after the
> swap if you need fresh configuration on the new component.
...
`.attrs()` is two overloads — callback form and object form — both of which feed back into the component's typed prop surface:

```tsx
const Button = rocketstyle()({ component: Element, name: 'Button' })
  .attrs({ tag: 'button' })

// `tag` now OPTIONAL at the call site — the default fills it in.
<Button />               // ✓ compiles
<Button tag="a" />       // ✓ still overridable, still `HTMLTags`-typed
```

Any key passed to `.attrs({ … })` becomes **optional** at the JSX call site (it has a runtime default — the consumer shouldn't be forced to pass it). This is the headline ergonomic of wrapping: pre-fill `component`, `tag`, etc. once and downstream call sites stay clean.

Callback form keeps **literal narrowing without `as const`**:

```tsx
.attrs(({ rootElement }) => ({
  tag: 'ul',                 // stays the literal 'ul', not widened to string
  contentAlignX: 'block',
}))
```

<Callout type="info">
Earlier majors required `as const` on every literal returned from an `.attrs()` callback. The callback overload now derives its return type from the component's resolved props (`DFP`), so literals narrow contextually. Object form already behaved this way. See PRs [#225](https://github.com/vitus-labs/ui-system/pull/225) (optional-at-call-site) and [#227](https://github.com/vitus-labs/ui-system/pull/227) (callback narrowing).
</Callout>
...
4. **Detects component swaps** — when `newOptions.component` is set and differs
   from the current component, drops `attrs` / `priorityAttrs` / `compose` /
   `filterAttrs` (these are component-shape-specific). Style-side state
   (themes, dimensions, statics, pseudo, etc.) is preserved.
5. Returns a fresh `rocketComponent()` with the merged configuration

This ensures immutability — the original component is never modified — while preventing prior prop-side configuration from silently leaking onto a new component when `.config({ component })` swaps it.
