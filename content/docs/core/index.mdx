...
### CSSEngineResult — Augmentable Type

`css()` returns `CSSEngineResult`, an **empty interface** core exposes for connectors to augment via TypeScript module declaration merging. Each connector declares its concrete result type so consumers' `` css`...` `` calls type to the engine they actually `init()`'d with — without core depending on any specific engine package.

```tsx
// inside @vitus-labs/connector-styler
import type { CSSResult as StylerCSSResult } from '@vitus-labs/styler'
...
Application code never imports `CSSEngineResult` directly — it resolves to the augmented shape of whichever connector is active in the project.
...
A subset of **40 text-level semantic tags** for elements that semantically contain text:
...
| `HTML_TEXT_TAGS` | `string[]` | 40 text-level semantic tags |
...
| `CSSEngineResult` | Augmentable empty interface — connectors merge their concrete `css\`...\`` result type into it via TS module declaration merging |
...
| `HTMLTextTags` | Union of 40 text semantic tags |
