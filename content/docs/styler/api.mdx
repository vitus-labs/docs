...
- **SSR:** Renders `<style precedence="medium">` for FOUC-free delivery
...
| `clearCache()` | `void` | Clear the dedup cache plus resolve-side caches (`insertCache`, `prepareCache`, `normCache`). Keeps already-injected DOM rules. |
| `clearAll()` | `void` | Everything `clearCache()` does, plus clear ssrBuffer, remove all DOM rules, and fire registered `clearCallbacks` so consumers (e.g. `styled.ts`'s static caches) reset. Full HMR reset. |
